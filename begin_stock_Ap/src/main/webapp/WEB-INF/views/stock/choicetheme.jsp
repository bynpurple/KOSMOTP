...
<c:if test="${emailresult == 1}">
	<script type="text/javascript">
		alert("성공적으로 요청되었습니다. 예상 소요 시간은 8 ~ 10 분입니다.");
	</script>	
</c:if>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#">차트 및 테마 분석</a></li>
    ...
    <section class="content">
	  ...
	  <div class="row">
						  ...
						  <tbody>
						  	...
						  	<c:forEach var="toplist" items="${topList}">
						  		...
						  		<c:if test="${toplist.getGap() == 0}">
			  			 		  <td>${toplist.getStockTheme()}</td>
								  <td onclick="location.href='financialInfo?stockcode=${toplist.getStockCode()}'" style="cursor:pointer;">${toplist.getStockName()}</td>
								  <td style="color:orange;">${toplist.getStockCurrent()}</td>
								  <td style="color:orange;">${toplist.getStockOpen()}</td>
								  <td>${toplist.getStockVolume()}</td>
								  <td style="color:orange;">${toplist.getGap()}</td>
						  		</c:if>
						  		</tr>
							</c:forEach>
						  </tbody>
		...
		<div class="col-12">
			<div class="box">
				<div class="box-body">
					<h2 class="page-header text-center mt-30 mb-30">
						<span class="text-bold text-yellow">EZ-STOCK</span> 테마별 정보 제공 <span class="text-bold text-yellow">서비스</span><br>
						<p class="font-size-18 mb-0">하단에 이메일을 입력하시고 요청 버튼을 누르시면 금일부터 지난 10일간의 각 테마별 정보가 담긴 파일을 이메일을 제공합니다.</p>
					</h2>
					<form action="requestedEmail" method="post">
						<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
						<div class="input-group margin" style="width:50%;margin-left:25%">
			                <div class="input-group-btn">
			                  <input type="submit" class="btn btn-danger" value="요청">
			                </div>
			                <!-- /btn-group -->
			                <input type="text" class="form-control" name="requestedEmail">
						</div>
					</form>
				</div>
			</div>
