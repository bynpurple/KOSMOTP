...
import com.spring.stock.vo.SMTPAuthenticatior;
...
public class StockServiceImpl implements StockService {
	...
	public void search(HttpServletRequest req, Model model) throws IOException {
		...
		public void asking_stock(HttpServletRequest req, Model model) {
			...
			List<Stock_buyVO> vo1 = dao.buying_stock(CompanyCode);
			List<Stock_sellVO> vo2 = dao.selling_stock(CompanyCode);
			model.addAttribute("vo",vo);
		...
		public void completedStock(HttpServletRequest req, Model model) {
			...
			List<OwnStockVO> list = dao.completedStock(map);
		...
		public void MyStock(HttpServletRequest req, Model model) {
			...
			List<OwnStockVO> list = dao.MyStock(username);
		...
		public void noticeList(HttpServletRequest req, Model model) {
			int pageSize = 50; 	// 글 최대 500개까지 저장 가능(500개 넘어가면 그 이후부터는 출력안됨 보이게 하려면 500을 다른 숫자로 변경하면 보임)
        ...
        public void hostForm(HttpServletRequest req, Model model) {
           ...
           model.addAttribute("vo", vo);
           int pageSize = 50;    // 표시할 유저 최대 500개까지 저장 가능(500개 넘어가면 그 이후부터는 출력안됨 보이게 하려면 500을 다른 숫자로 변경하면 보임)
           int pageBlock = 3;       // 한 블럭당 페이지 갯수 123 234(사실 이거 안쓰는데 그냥 지우기는 그래서[혹시 필요한 순간이 있을까봐] 안지움)
           int start = 0;          // 현재 페이지의 시작 글번호
           int end = 0;          // 현재 페이지의 마지막 글번호
           int number = 0;       // 출력용 글번호
		...
		public void sellOrder(HttpServletRequest req, Model model) {
			...
			int showCnt = Integer.parseInt(req.getParameter("showCnt")); //보유 주식수
		...
		public void confirmSession(HttpServletRequest req, Model model) {
			...
			System.out.println("stock username : " + username);
			System.out.println("stock password : " + password);
			model.addAttribute("username", username);
		...
		// ============== 아진 추가 2019.09.11 ================ 
		@Override
		public void ownStock(HttpServletRequest req, Model model) {
			 // 아이디 갖고오기
	        Authentication authentication = SecurityContextHolder.getContext().getAuthentication(); 
			User user = (User) authentication.getPrincipal(); 
			String strId = user.getUsername();
			
			List<Stock_completedVO> own = dao.ownStock(strId);
			model.addAttribute("own", own);
		}

		@Override
		public void requestedEmail(HttpServletRequest req, Model model) {
			String contents = req.getParameter("requestedEmail");
			System.out.println(contents);
			String email = "dlwo940305@gmail.com";
	        ...
	        try {
	             ...
	             msg.setFrom(new InternetAddress(contents));
	             InternetAddress to = new InternetAddress(email);         
	             msg.setRecipient(Message.RecipientType.TO, to);            
	             msg.setSubject("EZ-STOCK입니다 ", "UTF-8");            
	             msg.setText(contents, "UTF-8");            
	             
	             Transport.send(msg);
	             
			    model.addAttribute("emailresult", 1);
	  ...
	  // 매도 / 매수 등록 현황
      @Override
      public void buy_sell_reg(HttpServletRequest req, Model model) {
         String username = req.getParameter("username");
         
         List<Stock_buyVO> b_vo = dao.buy_reg(username);
         List<Stock_sellVO> s_vo = dao.sell_reg(username);
      ...
      // 등록 취소
      @Override
      public void cancel_buy_sell(HttpServletRequest req, Model model) {
         int div_cnt = Integer.parseInt(req.getParameter("div_cnt"));
         ...
         if(div_cnt ==0) {
            ...
            buy_map.put("stockcode", stockcode);
            buy_map.put("buy_price", buy_price);
            buy_map.put("buy_cnt", buy_cnt);
            
            int Cnt = dao.cancel_buy(buy_map);
            model.addAttribute("Cnt",Cnt);
         }else {
            String stockcode= req.getParameter("stockcode");
            String sell_price = req.getParameter("sell_price");
            String sell_cnt = req.getParameter("sell_cnt");
            
            Map<String,Object> sell_map = new HashMap<String,Object>();
            sell_map.put("stockcode", stockcode);
            sell_map.put("sell_price", sell_price);
            sell_map.put("sell_cnt", sell_cnt);
            
            int Cnt = dao.cancel_sell(sell_map);
            model.addAttribute("Cnt",Cnt);
         }
     } 
}
